...
class ValidationSessionGuiTests(unittest.TestCase):
    ...
    def test_validation_session_generate_handoff_json_and_prompt(self) -> None:
        ...
        self.assertIn("Pass Count: 4", payload["handoff_markdown"])
        self.assertIn("Fail Count: 0", payload["handoff_markdown"])
        self.assertIn("Pending Count: 3", payload["handoff_markdown"])
    ...
    def test_validation_session_generate_handoff_uses_latest_form_values(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertIn("Pass Count: 6", payload["handoff_markdown"])
        self.assertIn("Fail Count: 1", payload["handoff_markdown"])
